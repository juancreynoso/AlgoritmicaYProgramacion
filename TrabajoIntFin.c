...
char nombreArchivo[100]; //Variable que almacena el nombre del archivo, el cual va a ser un anio sobre el cual se va a trabajar. Luego concatenado con ".dat"
int salio;
...
void ArchivoParaArreglo (char nomArch[100], TData *arr);  //Accion que pasa un archivo a un arreglo, manteniendo el archivo original.
void Cargar (char nomArch[100], int anio);  //Carga de datos en un registro.
void Suprimir (char nomArch[100], int anio);    //Supresion de datos de un registro del archivo.
void Modificar (char nomArch[100], int anio);   //Accion que permite modificar los campos del registro que el usuario desee, si es que existe.
void MostrarRegAct (char nomArch[100]);       //Accion que muestra los registos activos.
...
void MaximaTemperatura (char nomArch[100]);           //Accion que muestra el/los dias que tuvieron la maxima temperatura del anio.
...
void CopiaDeSeguridad (char nomArch[100], int anio);     //Realiza una copia de seguridad del archivo, con todos los datos cargados hasta el momento, los cuales no esten borrados logicamente
void MostrarReg (TData a, int pos);                             //Muestra un registro deseado por el usuario.
void CrearArchivo(int *s, char nomArch[100], int *anio);
...
int main (){  //Inicio del algoritmo
    ...
    //Funcionamiento explicado en el pdf subido junto al archivo de implementacion 
    salio = 0;
    do{
    
       CrearArchivo(&salio, nombreArchivo, &a); 
...
void CrearArchivo(int *s, char nomArch[100], int *anio){
    ...
    fflush(stdin);
    scanf("%s", nomArch); //Pedimos el nombre del archivo

    strcat(nomArch, ".dat");    //Concatenamos el nombre del archivo con ".dat
    f = fopen (nomArch, "rb");  //Abrimos en este modo 
    ...
    if (nuevo == 'N' || nuevo == 'n'){     //Si el usuario desea trabajar con un nuevo archivo
        ...
        if(f == NULL){          //Si el archivo no esta creado
            ...
            f = fopen (nomArch, "w+b");    //Se crea el archivo en modo escribir
            printf("Ingrese el anio con el que va a trabajar: ");
            fflush(stdin);
            scanf("%d", &(*anio));      //Pedimos el anio
            rG.ddmmyyyy = *anio;       
            rG.borrado = 1;  
        ...
        if(f == NULL){
            ...
            *anio = (((rG.ddmmyyyy/10000) * 10000 ) - rG.ddmmyyyy) * -1;  
            *s = 1;
            fclose(f);
...
void ArchivoParaArreglo (char nomArch[100], TData *arr){
...
void Cargar (char nomArch[100], int anio){
...
void Suprimir (char nomArch[100], int anio){
    ...
    if(tam - 36 == 0){   //Si tam ==  36 significa que el unico registro que hay en el archivo es el "ficticio"
...
void Modificar (char nomArch[100], int anio){
...
void MostrarRegAct(char nomArch[100]){
    ...
    if (tam - 1 == 0){      //Si el archivo esta vacio
...
void MaximaTemperatura (char nomArch[100]){
...
void CopiaDeSeguridad (char nomArch[100], int anio){
    ...
    char nomCopia[100];    //Variable que almacena el nombre de la copia de seguridad
